...
def miles_filename_conversion(num):
    ...
    'm0042V'
    """
    min_miles_id, max_miles_id = [1, 985]  # MILES library size
    missing_miles = 970  # template removed from MILES library
    valid = ((type(num) is int) and (min_miles_id <= num) and
             (num <= max_miles_id) and (num != missing_miles))
    if not valid:
        raise Exception("Invalid MILES template id number: {}".format(num))
    return "m{:04d}V".format(num)
