...
# skip finished runs
param_pattern = "ngc1600mitchell-tile-(?P<range>[\d._]+)-gh_params.txt"
starting_param_files, param_groups = re_filesearch(param_pattern)
param_ranges = [f.groups()[0] for f in param_groups]
chisq_pattern = "ngc1600mitchell-tile-(?P<range>[\d._]+)-chisq_dof.txt"
...
for range_iter, fit_range in enumerate(knobs["fit_range"]):
    ...
    # mult_degree = int((fit_stop - fit_start)/140.0)  # (1 deg)/(140 A)
    ...
    #                       interpolated_ir, logscales[0],
    #                       template_range))
    # template_output = {}
    # template_output["templates"] = prepared_templates
    # template_output["templates_waves"] = prepared_template_waves
    template_title = ("{}-allbins-{}_{}"
                      "".format(knobs['name'], fit_start, fit_stop))
    # for output_name, data in template_output.iteritems():
    #     output_filename = "{}-{}.txt".format(template_title, output_name)
