...
# convolve, but not interpolate, all templates with single IR
interpolated_ir = compute_ir(template_wavelengths, binned_ir[0, ...])
prepared_templates, prepared_template_waves = (
    prepare_templates(template_spectra, template_wavelengths,
                      ...
                      logscales[0], template_range))

# fit spectra
try:
    int(knobs["multiplicative_degree"])  # is single number
    knobs["multiplicative_degree"] = [knobs["multiplicative_degree"]]
except:
    pass  # already in list form
num_degrees = len(knobs["multiplicative_degree"])

# skip finished runs
param_pattern = "ngc1600mitchell_findKGMtemps-(?P<deg>\d+)-gh_params.txt"
starting_param_files, param_groups = re_filesearch(param_pattern)
param_degrees = [f.groups()[0] for f in param_groups]
chisq_pattern = "ngc1600mitchell_findKGMtemps-(?P<deg>\d+)-chisq_dof.txt"
starting_chisq_files, chisq_groups = re_filesearch(chisq_pattern)
chisq_degrees = [f.groups()[0] for f in chisq_groups]
finished = [int(r) for r in param_degrees if r in chisq_degrees]


full_bin_wavelengths = binned_data[0, 2, :]  # assume samplings identical
for deg_iter, mult_degree in enumerate(knobs["multiplicative_degree"]):
    mult_degree = int(mult_degree)
    # skip finished runs
    if mult_degree in finished:
        print ("mult {}/{}: {} is already fit; skipping..."
               "".format(1 + deg_iter, num_degrees, mult_degree))
        continue
    # proceed with fits
    fit_range = knobs["fit_range"]
    fit_start, fit_stop = fit_range
    print ("fitting degree {}/{}: {}"
           "".format(1 + deg_iter, num_degrees, mult_degree))
    print "range {} - {}".format(fit_start, fit_stop)
    out_of_bounds = ((full_bin_wavelengths < fit_start) |
                     (fit_stop < full_bin_wavelengths ))
    fit_data = binned_data[:, :, ~out_of_bounds]
    fit_wavelengths = fit_data[0, 2, :]  # assume samplings identical
    masked = in_range_union(fit_wavelengths, knobs["mask_observed"])
    ...
    # log_galaxy_initial = np.log(fit_wavelengths.min())
    ...
    # run fits
    run_output["wavelengths"].append(fit_wavelengths)
    for bin_index, bin_number in enumerate(bin_numbers):
        print ("fitting deg {}: bin{:02d} with {} templates"
               "".format(mult_degree, bin_number,
                         prepared_templates.shape[0]))
        bin_output = {}
        bin_spectrum = fit_data[bin_index, 0, :]
        bin_noise = fit_data[bin_index, 1, :]
        bin_badpixels = fit_data[bin_index, 3, :].astype(bool)
        ...
        if num_sims > 1:
            ...
            run_output["gh_params_errors"].append(sim_paramerror)
            covmat = np.nan*np.ones((num_moments, num_moments))
        # write output
        fit_title = ("{}-bin{:02d}-{}"
                     "".format(knobs['name'], bin_number, mult_degree))
    ...
    for output_name, data in run_output.iteritems():
        output_filename = ("{}-{}-{}.txt".format(knobs['name'],
                                                 mult_degree, output_name))
        np.savetxt(output_filename, np.asarray(data).T, delimiter='  ')