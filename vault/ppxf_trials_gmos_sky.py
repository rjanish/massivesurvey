...
from MassiveKinematics import (speed_of_light, prepare_templates_hist,
...
binned_data, s2n, logscales = [], [], []
for spectrum_path in binned_spectra_paths:
    hdu = fits.open(spectrum_path)
    data = hdu[0].data
    hdu.close()
    spectrum, noise, wavelengths, bad_pixels = data
    norm = np.median(spectrum)
    data[0, :] = data[0, :]/norm  # convenience normalize spectrum
    data[1, :] = data[1, :]/norm  # convenience normalize noise
...
# get sky
sky_paths = re_filesearch(knobs['sky_pattern'],
                          directory=knobs['sky_dir'])[0]
sky_templates = np.zeros((len(sky_paths), wavelengths.size))
for sky_index, sky_path in enumerate(sky_paths):
    sky_spectrum = utl.fits_quickread(sky_path)[0][0]
    sky_header = utl.fits_quickread(sky_path)[1][0]
    sky_waves = utl.fits_getcoordarray(sky_header)[0]
    sky_func = interp1d(sky_waves, sky_spectrum)
    sky_samples = sky_func(wavelengths)
    sky_templates[sky_index, :] = sky_samples/np.median(sky_samples)

# output bin numbers
bin_names = [os.path.splitext(os.path.basename(path))[0].split('_')[0]
             for path in binned_spectra_paths]
binnames_filename = '{}-binnames.txt'.format(knobs["name"])
np.savetxt(binnames_filename, bin_names, fmt='%s')
...
template_index = [[os.path.splitext(os.path.basename(path))[0]]
                  for path in template_paths]  # use star filenames
...
for fit_start, fit_stop in knobs["fit_range"]:
    ...
    if 'mask_observed' in knobs:
        masked = in_range_union(bin_wavelengths, knobs["mask_observed"])
    else:
        masked = np.zeros(bin_wavelengths.shape, dtype=bool)
    masked = masked | out_of_bounds
    guess = ([knobs["v_guess"], knobs["sigma_guess"]] +
             [knobs["hn_guess"]]*int(knobs["moments_to_fit"] - 2))
    bias = knobs['bias']
    add_degrees = knobs['additive_degree']
    mult_degrees = knobs['multiplicative_degree']
    try:
        continuum_iter = zip(map(int, add_degrees), map(int, mult_degrees))
    except:
        continuum_iter = [(int(add_degrees), int(mult_degrees))]
    for add_degree, mult_degree in continuum_iter:
        num_moments = int(knobs['moments_to_fit'])
        run_only = ["masked", "wavelengths", "noise",
                    "bin_number", "bin_name", "gh_params_errors"]
        run_and_bin = ["gh_params", "tempweights", "model",
                       "mult_params", "mult_model", "add_params",
                       "add_model", "chisq_dof", "spectrum", "skyweights"]
        ...
        #     prepare_templates_hist(template_spectra, template_wavelengths,
        ...
        nonzero = np.ones(template_spectra.shape[0], dtype=bool)
        ...
        print 'fitting with {} templates'.format(
            selected_template_spectra.shape[0])
        # run fits
        for bin_index, bin_name in enumerate(bin_names):
            print ("fitting bin{} {}: {}_{} add{} mult{}"
                   "".format(bin_index, bin_name, fit_start, fit_stop,
                             add_degree, mult_degree))
            bin_output = {}
            bin_spectrum = binned_data[bin_index, 0, ~out_of_bounds]
            bin_noise = binned_data[bin_index, 1, ~out_of_bounds]
            bin_badpixels = binned_data[bin_index, 3, ~out_of_bounds].astype(bool)
            bin_ir = binned_ir[bin_index, ...]
            bin_sky = sky_templates[:, ~out_of_bounds]
            bad_pixels = masked[~out_of_bounds] | bin_badpixels
            good_pixels_indicies = np.nonzero(~bad_pixels)[0]
            run_output["wavelengths"].append(bin_wavelengths)
            run_output["bin_number"].append(bin_index)
            run_output["bin_name"].append(bin_name)
            ...
            prepared_templates, prepared_template_waves = (
                prepare_templates_hist(selected_template_spectra,
                                  template_wavelengths, knobs["template_fwhm"],
                                  interpolated_ir, logscales[bin_index],
                                  template_range))
            num_templates = prepared_templates.shape[0]
            bin_output["templates"] = prepared_templates
            bin_output["templates_waves"] = prepared_template_waves
            bin_output["sky"] = bin_sky
            # actual fit
            log_template_initial = np.log(prepared_template_waves.min())
            log_galaxy_initial = np.log(bin_wavelengths[~out_of_bounds].min())
            velocity_offset = (log_template_initial -
                               log_galaxy_initial)*speed_of_light
            ...
            ppxf_fitter = ppxf(prepared_templates.T,  # templates in cols
                               ...
                               moments=num_moments, degree=add_degree,
                               vsyst=velocity_offset, mdegree=mult_degree,
                               plot=False, quiet=True, sky=bin_sky.T)
            # re-scale noise
            bin_noise *= np.sqrt(ppxf_fitter.chi2)
            ppxf_fitter = ppxf(prepared_templates.T,  # templates in cols
                               bin_spectrum, bin_noise, velscale, guess,
                               goodpixels=good_pixels_indicies, bias=bias,
                               moments=num_moments, degree=add_degree,
            ...
            run_output["tempweights"].append(ppxf_fitter.weights[:num_templates])
            run_output["skyweights"].append(ppxf_fitter.weights[num_templates:])
            ...
            if num_sims > 1:
                ...
                for trial in xrange(num_sims):
                    ...
                    simulated_galaxy = bestfit_model + noise_draw*bin_noise
                    ...
                    bin_output["tempweights"][trial, ...] = sample_fitter.weights[:num_templates]
                    bin_output["skyweights"][trial, ...] = sample_fitter.weights[num_templates:]
            ...
            # write output
            fit_title = ("{}-{}-{}_{}-add{}_mult{}"
                         "".format(knobs['name'], bin_name, fit_start,
                                   fit_stop, add_degree, mult_degree))
            ...
            plot_results(bin_spectrum, bestfit_model, bin_noise,
                         bin_wavelengths[~out_of_bounds],
            ...
            write_results_files(ppxf_fitter.sol, sim_paramerror, chisq_dof,
                                ppxf_fitter.weights[:num_templates], covmat,
                                template_index,
                                "{}-results.txt".format(fit_title), fit_title,
                                bestfit_addpoly_weights,
                                bestfit_multpoly_weights)
        for output_name, data in run_output.iteritems():
            output_filename = ("{}-{}_{}-add{}_mult{}-{}.txt"
                               "".format(knobs['name'], fit_start, fit_stop,
                                         add_degree, mult_degree, output_name))
            try:
                np.savetxt(output_filename, np.asarray(data).T, delimiter='  ')
            except:
                np.savetxt(output_filename, np.asarray(data).T,
                           delimiter='  ', fmt='%s')