...
num_ranges = len(knobs["fit_range"])

# skip finished runs
param_pattern = "{}-(?P<range>[\d._]+)-gh_params.txt".format(knobs['name'])
starting_param_files, param_groups = re_filesearch(param_pattern)
param_ranges = [f.groups()[0] for f in param_groups]
chisq_pattern = "{}-(?P<range>[\d._]+)-chisq_dof.txt".format(knobs['name'])
starting_chisq_files, chisq_groups = re_filesearch(chisq_pattern)
chisq_ranges = [f.groups()[0] for f in chisq_groups]
finished = [map(float, r.split('_'))
            for r in param_ranges if r in chisq_ranges]


full_bin_wavelengths = binned_data[0, 2, :]  # assume samplings identical
for range_iter, fit_range in enumerate(knobs["fit_range"]):
    # skip finished runs
    if fit_range in finished:
        print ("range {}/{}: {}-{} is already fit; skipping..."
               "".format(1 + range_iter, num_ranges, *fit_range))
        continue
    # proceed with fits
    fit_start, fit_stop = fit_range
    print ("fitting range {}/{}: {}-{}"
           "".format(1 + range_iter, num_ranges, fit_start, fit_stop))
    ...
    mult_degree = int((fit_stop - fit_start)/200.0)  # (1 deg)/(140 A)
    print "multiplicative degree: {}".format(mult_degree)
